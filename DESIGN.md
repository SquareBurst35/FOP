...
Cada um dos cinco elementos paranormais (Sangue, Morte, Conhecimento, Energia, Medo) tem um glifo original de 20px (`elementGlyph()` em `app.js`, traço único, `stroke-linecap/linejoin: round`), desenhado a partir da descrição geométrica do símbolo real de cada elemento no universo de Ordem Paranormal (Ordem Paranormal Wiki, "Símbolos Ocultistas") — nunca copiado de arte oficial, sempre uma interpretação original em linha. Substitui a antiga borda lateral colorida (`border-left`) nos cartões de ritual e nos botões de filtro por elemento: o glifo é a identidade visual do elemento, a cor é reforço, não o contrário.
