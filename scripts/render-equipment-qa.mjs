...
import { BODY_ATLAS, FEMALE_BODY, paintPaperdoll, paperdollImagePaths } from '../paperdoll-renderer.js';
...
const bare=render([],{}, {appearance}).c.getContext('2d').getImageData(0,0,420,600).data;
for(let start=0;start<ITEMS.length;start+=15) {
 const cases=ITEMS.slice(start,start+15).map((item,i)=>{
  const {c,placements}=render([{...item,quantity:1}],{}, {appearance});
  const art=artForItem(item),recipe=compositionFor(art);
  fs.writeFileSync(path.join(output,`${appearance}-${String(start+i).padStart(3,'0')}.png`),c.toBuffer('image/png'));
  ...
  report.push({appearance,index:start+i,id:item.id,name:item.name,kind:recipe.kind,variant:variantFor(art).key,visibility:recipe.visibility,placements:placements.length,changedPixels});
 ...
 });sheet(cases,appearance+'-items-'+String(start/15).padStart(2,'0'));
}
const combined=sets.map((names,i)=>{const {c}=render(names.map(own),{}, {appearance});fs.writeFileSync(path.join(output,`${appearance}-set-${i}.png`),c.toBuffer('image/png'));return {canvas:c,label:`${i}. ${names.join(' + ')}`};});
sheet(combined,appearance+'-combinations');
const secondary=ITEM_ART.filter(a=>a.attachment==='hand').map(a=>{
 const item=own(a.name),{c}=render([item],{weapon:'',secondary:item.id},{appearance});return {canvas:c,label:a.name};
});
for(let i=0;i<secondary.length;i+=15)sheet(secondary.slice(i,i+15),appearance+'-left-'+Math.floor(i/15));
}
...
console.log(`Rendered ${report.length} individual items, ${sets.length} sets, both-hand checks and custom backpack into ${output}`);
