...
assert.equal(ITEMS.length,225);assert.equal(ITEM_ART.length,225);assert.equal(ORIGINAL_ITEM_ART.length,165);
assert.equal(new Set(ITEMS.map(i=>i.id)).size,225);
...
assert.equal(new Set(ITEM_UPGRADES.map(u=>u.id)).size,ITEM_UPGRADES.length);assert.equal(ITEM_UPGRADES.length,63);
...
assert.equal(report.length,450);
...
console.log('225 catalog entries, 63 attached improvements, original 165 sprites and 450 raster coverage cases passed.');
