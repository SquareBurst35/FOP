...
assert.equal(ITEMS.length,230);assert.equal(ITEM_ART.length,230);assert.equal(ORIGINAL_ITEM_ART.length,165);
assert.equal(new Set(ITEMS.map(i=>i.id)).size,230);
...
assert.equal(report.length,460);
...
console.log('230 catalog entries, 63 attached improvements, original 165 sprites and 460 raster coverage cases passed.');
